...
//set next order date as date value
let currentDate = new Date();
let nextAvailableDeliveryDate = findDeliveryDate(currentDate, true);
let dateFormat = `${nextAvailableDeliveryDate.getDate()}/${nextAvailableDeliveryDate.getMonth() + 1}/${nextAvailableDeliveryDate.getFullYear()} - ${getWeekDay(nextAvailableDeliveryDate.getDay() - 1)}`;
dateInputField.value = dateFormat
...
//Clicking the calendar Date button
calendarDateButtonCollection.addEventListener("click", (e) => {
  if (
    e.target.parentNode.tagName === "TR" &&
    e.target.parentNode !== dayTable.querySelector("tr:first-child")
  ) {
    dateInputField.value = e.target.getAttribute("date");
    calendarContainerElement.style.display = "none"
  }
});
